...
- Optional: `MOONI_MEDIA_DIR` (a dedicated folder for the Photos-style media library; unset → `/api/media/*` not registered). `setup.sh` prompts for it during setup; you can also add it to `~/.mooni/config.env` by hand (the dir must already exist). `MOONI_REDIS_ADDR`/`MOONI_REDIS_PASSWORD` (Redis cache; unset → in-process cache). Thumbnails are cached on disk under `~/.mooni/thumbs/`.
- Install is split in two phases. `agent/install.sh` is a **non-interactive bootstrap** (safe for `curl | bash`, no reads): downloads a prebuilt binary from GitHub Releases (`hideffrand/mooni`), sha256-verifies it, installs to `--bin-dir` (default `~/.local/bin`), and copies `setup.sh` + `uninstall.sh` to `~/.mooni/bin/`. Run from a checkout it detects `go.mod`+`main.go` and builds from source instead. It then invokes the interactive `agent/setup.sh` (or `~/.mooni/bin/setup.sh`) by default. Releases come from `.github/workflows/release.yml` on a `v*` tag push (linux amd64/arm64 tarballs + `checksums.txt`).
- Interactive setup + pairing: `setup.sh` (generates key, writes `~/.mooni/config.env` mode 600, optional systemd service, optionally adds a scoped passwordless-sudo rule for `systemctl reboot`/`poweroff` in `/etc/sudoers.d/mooni-power`, prints QR/pairing code). On WSL it detects the env: skips the systemd service when systemd isn't PID 1, skips the power-control sudoers rule (can't reboot Windows from WSL), and prints mirrored-networking/portproxy guidance so the phone can reach the server.
- Uninstall: `~/.mooni/bin/uninstall.sh` (curl-installed) or `agent/uninstall.sh` (checkout); both resolve the binary next to the script, in `~/.local/bin`, or on PATH. Stops/removes the systemd service and sudoers rule, removes the binary and `~/.mooni` config; lists the paired storage folder and only deletes it if the user picks that option AND types `DELETE` - never automatically.
