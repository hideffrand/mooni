    ...
    >
      ...
      {DOTS.map(([x, y], i) => (
        <circle
          ...
          cy={y}
          r={i % 3 === 0 ? 2.2 : 1.4}
          fill={i % 5 === 0 ? "var(--accent-3)" : "var(--accent-2)"}
          className="twinkle"
...
export function SignalLink({ className = "" }: { className?: string }) {
  return (
    ...
    >
      {/* server */}
      <g stroke="var(--text-3)" strokeWidth="1.4" fill="none">
        <rect x="14" y="24" width="34" height="42" rx="3" />
        <line x1="14" y1="37" x2="48" y2="37" />
        <line x1="14" y1="50" x2="48" y2="50" />
      </g>
      <circle cx="21" cy="30.5" r="1.6" fill="var(--accent)" className="twinkle" />
      <circle cx="21" cy="43.5" r="1.6" fill="var(--accent)" className="twinkle" style={{ animationDelay: "600ms" }} />
      ...
      <circle cx="287" cy="45" r="1.6" fill="var(--accent-3)" className="twinkle" style={{ animationDelay: "1.1s" }} />
    </svg>
  );
}
...
export function QRScan({ className = "" }: { className?: string }) {
  return (
    <div
      className={`relative grid h-11 w-11 grid-cols-6 grid-rows-6 gap-[2px] overflow-hidden rounded-md border border-[var(--border)] bg-[var(--card-2)] p-1.5 ${className}`}
    >
      {QR_PATTERN.map((on, i) => (
        <span
          key={i}
          className={`rounded-[1px] ${on ? "bg-[var(--text-3)]" : "bg-transparent"}`}
        />
      ))}
      <span
        aria-hidden
        className="scan-beam pointer-events-none absolute inset-x-0 h-3 bg-gradient-to-b from-transparent via-[var(--accent-2)]/70 to-transparent"
      />
    </div>
  );
}
